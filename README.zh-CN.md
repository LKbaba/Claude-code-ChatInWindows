...
| 版本 | 日期 | 亮点 |
|------|------|------|
| **v4.1.8** | 2026-09-25 | **模型配置**：可隐藏模型、按模型设置 effort（Auto/Low/Med/High/xHigh/Max），设了 `CLAUDE_CODE_EFFORT_LEVEL` 也能生效；5h 额度圆圈显示重置时间；加回 Opus 5、Opus 4.7；新增 Codex 开关（`codex exec`），替代已失效的 Codex MCP 模板，请删掉旧的 `codex-official`；MCP 配置不再写明文密钥；清理临时 MCP 目录；恢复检查点可撤销 |
| **v4.1.7** | 2026-09-25 | 新增 Opus 5.5 模型（1M 上下文）；**修复 Windows 上附加系统提示词从未送达模型**（v4.1.5 起的运行约束和 MCP 说明一直丢失）；新模型恢复 Todo 清单并加入任务清单提示；识别 CLI 新工具图标（PowerShell、LSP、Monitor、Workflow、Task 系列）；消除 DEP0190 警告；Sonnet 5 价格改为 $2/$10 |
| **v4.1.6** | 2026-09-23 | 新增 Fable 5.1 模型（1M 上下文，`[1m]` 注入）；**修复切换 API 端点/改设置时全局 MCP 服务器掉到项目级**的严重缺陷；新增 API Key 清除按钮 + 掩码预览与明文切换；从选择器隐藏 Opus 4.7；移除失效的 MCP 配置目标机制 |
| **v4.1.5** | 2026-08-16 | 新增 headless 运行时约束提示——告知模型 CLI 每次回复后即退出，后台进程/定时器/轮询循环无法跨轮次存活；引导长任务改用系统调度器或文件持久化进度 |
