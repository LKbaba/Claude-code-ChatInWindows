...
| 版本 | 日期 | 亮點 |
|------|------|------|
| **v5.0.8** | 2026-06-15 | **預設啟用 1M token 上下文視窗**——長工作階段會在約 165K token 處悄無聲息地撞上 `Prompt is too long`，根因是啟動的模型其實從未帶上 1M 視窗（裸 `--model claude-opus-4-8` = 200K），而進度條分母卻可能被設成 1M 掩蓋了真實上限，加上自動壓縮已預設關閉、沒有兜底。現在驅動會在 spawn 時**以及**工作階段中途 `/model` 切換時，給支援 1M 的模型（Opus 4.6/4.7/4.8、Sonnet 4.6）的 `--model` 追加 `[1m]` 後綴，真實視窗由 200K 升到 1M（已 GA，無需 beta header）。透過 `enable1MContext` 開關控制。⚠️ 1M 僅在 Max/Team/Enterprise + Opus 下含在訂閱內 |
| **v5.0.7** | 2026-06-16 | 修復在全新安裝的電腦上外掛「無法執行」的問題——全新機器會連續彈出兩個啟動門（工作區信任 + bypass 權限警告），而驅動只應答了第一個、判定就緒後即停止掃描，導致使用者首條訊息的 Enter 落在第二個門的預設「No, exit」上，`claude` 退出碼 1 並不斷重啟迴圈。現在兩類門各自獨立追蹤、並在 spawn 後約 25 秒內持續掃描，確保兩個門都被應答 |
| **v5.0.6** | 2026-06-15 | 修復編輯技能/掛鉤/任意 `.claude/` 檔案時回合卡死約 69 秒的問題（CLI 對自身配置的「編輯確認選單」`bypassPermissions` 也不自動通過、PTY 驅動答不了，現自動選「本工作階段授權」一次解決後續編輯）。預設停用框架的有損自動壓縮（`DISABLE_AUTO_COMPACT=1`，手動 `/compact` 仍可用）。恢復 subagent（Agent/Task）工具步驟的內聯顯示且不重複計費 token。Context Window 預設 200K→400K |
| **v5.0.5** | 2026-06-15 | 修復 `/compact` 期間或切換工作階段後訊息被靜默吞掉的問題（compact 模式旗標現在會在每次切換工作階段時重設；壓縮進行中傳送會明確提示「正在處理中，請稍候」而不再遺失）。同時把過大的 QUESTION 選項卡標題（`.ask-question-header` 14px→13px）縮小，使其不再比對話徽章高出一截 |
| **v5.0.4** | 2026-06-15 | 打包修復：v5.0.3 的 VSIX 因 `--no-dependencies` 漏打了原生 `node-pty`，導致裝得上但聊天面板打不開。現已正確打包 node-pty 的 win32-x64 二進位檔 |
| **v5.0.3** | 2026-06-15 | 互動式選項卡片：停用原生 `AskUserQuestion`，Claude 改用圍欄 ` ```ask ` 區塊輸出問題，前端渲染成可點擊的單選/多選卡片，點擊即把所選 label 作為下一輪訊息注入。修復 ask 區塊跟在工具呼叫後（`stop_reason: null`）導致的「仍在處理」鎖死，並優化卡片版面與字體 |
