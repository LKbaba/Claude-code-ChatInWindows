...
> **版本**: v1.3
> **日期**: 2026-03-29
> **状态**: Phase 1 已完成，Phase 1.1（UI 打磨 + 简化）已完成，Phase 2-4 待定
> **目标版本**: v4.0.2（Phase 1）、v4.0.3-4.0.4（Phase 1.1 UI 打磨+简化）、v4.1+（Phase 2-4）
...
### Phase 1.1 交付物（v4.0.3 UI 打磨 + v4.0.4 UI 简化）

| 功能 | 优先级 | 状态 | 架构变更 |
|------|--------|------|---------|
| Hooks 按钮带计数显示 | P0 | ✅ 已完成 | 初始 "Hooks: All"，动态更新 |
| Hook 项目启用/禁用药丸标签 | P0 | ✅ 已完成 | `.skill-state-btn` 绿/红药丸 |
| Scope 简化为两级 | P0 | ✅ 已完成 | Project（上）+ Global（下），移除 Project Local |
| 移除 Add/Edit 功能 | P0 | ✅ 已完成 | hooks 为代码型配置，仅保留 Templates + Delete |
| Templates scope 选择器 | P1 | ✅ 已完成 | 用户可选择 Project/Global scope 应用模板 |
| 默认写入 project scope | P1 | ✅ 已完成 | 模板默认写入 `.claude/settings.json` |
...
```
┌─────────────────────────────────────────────┐
│  Hooks Management          [Templates] [Refresh] ✕ │
├─────────────────────────────────────────────┤
│  Apply to: [Project ▼]                      │  ← 模板 scope 选择器（仅展开模板时可见）
│  ┌─────────────────────────────────────┐    │
│  │  Completion Notification (Stop)     │ Use│  ← 模板卡片
│  └─────────────────────────────────────┘    │
│                                             │
│  Loaded 2 hook(s)                           │  ← .hooks-status
│                                             │
│  ▼ Project (.claude/settings.json) (1)      │  ← project + project-local 合并显示
│  ┌─────────────────────────────────────┐    │
│  │  Stop: 任务完成通知                 │    │
│  │  [Enabled]                  [Delete]│    │
...
│  └─────────────────────────────────────┘    │
│                                             │
│  ► Global (~/.claude/settings.json) (1)     │
│  ┌─────────────────────────────────────┐    │
│  │  PreToolUse: 记录工具调用           │    │
│  │  [Enabled]                  [Delete]│    │
│  │  matcher: * | cmd: bash ~/log.sh    │    │
│  └─────────────────────────────────────┘    │
└─────────────────────────────────────────────┘
```
...
- **[Templates]**：展开模板区域，带 scope 选择器（Project/Global），点击 Use 一键应用
- **[Delete]**：直接删除（无确认弹窗 — VS Code webview 的 `confirm()` 不可靠）
- **Scope 分组**：Project（上方，合并 project + project-local）、Global（下方）

> **设计决策（v4.0.4）**：移除了 Add 和 Edit 按钮。Hooks 本质上是代码型配置（shell 命令 + matcher），应由用户在 settings 文件中编写或通过模板一键应用，不应通过 GUI 手动输入。三级 scope 简化为两级显示（Project / Global），project-local 归入 Project。
...
#### 5.7.2 Hook 项目布局（已实现 — v4.0.3+）

采用与 Skills 一致的 `.skill-item` → `.skill-content` → `.skill-header-row` 结构：
- 启用/禁用：`.skill-state-btn` 绿/红药丸
- 名称：`.skill-name-text` 显示 `{event}: {description}`
- 描述：`.skill-description` 显示 `matcher: ... | cmd: ...`
- 操作按钮：仅 Delete（移除了 Edit，hooks 是代码型配置，不应通过 GUI 编辑）

#### 5.7.3 Scope 分组（已简化 — v4.0.4）

**v4.0.3 设计**：三级分组（Global / Project / Project Local）
**v4.0.4 简化**：两级分组 — **Project（上方）** + **Global（下方）**

- `project` 和 `project-local` scope 的 hooks 合并显示在 "Project" 分组下
- 使用 `skill-category` + `skill-category-header` CSS class 体系

| 显示分组 | 包含的 scope | category-path |
|---------|-------------|---------------|
| Project | `project` + `project-local` | `.claude/settings.json` |
| Global | `global` | `~/.claude/settings.json` |

#### 5.7.4 Templates scope 选择器（新增 — v4.0.4）

模板区域新增 "Apply to" 下拉选择器，用户可选择将模板应用到 Project 或 Global scope。默认写入 `.claude/settings.json`（project scope）。
...
| 功能 | 移除原因 |
|------|---------|
| **Add 按钮 + 表单** | Hooks 是代码型配置（shell 命令），不应通过 GUI 手动输入 |
| **Edit 按钮** | 同上，用户应直接编辑 settings 文件 |
| **Project Local 分组** | 简化为两级（Project / Global），project-local 归入 Project |

#### 5.7.6 涉及文件

| 文件 | 变更内容 |
|------|---------|
| `src/ui-v2/getBodyContent.ts` | 按钮默认 `Hooks: All`；移除 Add 按钮和表单；Templates 增加 scope 选择器 |
| `src/ui-v2/ui-script.ts` | 移除 editHook/showAddHookForm/submitHookForm/cancelHookForm；简化为两级 scope 分组 |
| `src/providers/ClaudeChatProvider.ts` | `_applyHookTemplate` 支持前端传入 scope，默认 project |
...
### Phase 1.1：Hooks UI 打磨 + 简化（v4.0.3 + v4.0.4）— ✅ 已完成

| 步骤 | 任务 | 文件 | 状态 |
|------|------|------|------|
| 1 | Hooks 按钮文本：`Hooks ▼` → `Hooks: All ▼` / `Hooks: N ▼` | `getBodyContent.ts`、`ui-script.ts` | ✅ |
| 2 | Hook 项目：checkbox → `.skill-state-btn` 绿/红药丸 | `ui-script.ts` | ✅ |
| 3 | Hook 项目布局：采用 `.skill-content` → `.skill-header-row` 结构 | `ui-script.ts` | ✅ |
| 4 | Scope 简化：三级 → 两级（Project + Global） | `ui-script.ts` | ✅ |
| 5 | 移除 Add/Edit：hooks 为代码型配置，不需 GUI 输入 | `getBodyContent.ts`、`ui-script.ts` | ✅ |
| 6 | Templates scope 选择器：支持 Project/Global 选择 | `getBodyContent.ts`、`ui-script.ts`、`ClaudeChatProvider.ts` | ✅ |
| 7 | 默认写入 project scope（非 project-local） | `ClaudeChatProvider.ts` | ✅ |
