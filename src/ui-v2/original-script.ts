export const originalScript = `
		...
		window.addEventListener('message', event => {
			...
			switch (message.type) {
				...
				case 'extensionVersion':
					// Update the version display in the header
					const headerElement = document.querySelector('.header');
					if (headerElement) {
						const spans = headerElement.querySelectorAll('span');
						spans.forEach(span => {
							if (span.style.fontSize === '12px' && span.textContent && span.textContent.startsWith('v')) {
								span.textContent = 'v' + message.version;
							}
						});
					}
					break;
			...
			if (message.type === 'settingsData') {
				...
				// Load MCP settings
				document.getElementById('mcp-enabled').checked = message.data['mcp.enabled'] || false;
				document.getElementById('mcpOptions').style.display = message.data['mcp.enabled'] ? 'block' : 'none';
		...
		// Set ready status after a short delay to ensure DOM is fully loaded
		setTimeout(() => {
			updateStatusWithTotals();
		}, 100);
			...
			if (data.rows && data.rows.length > 0) {
				data.rows.forEach(row => {
					...
					if (currentStatsTab === 'blocks') {
						...
						html += '<td>' + (row.models && row.models.length > 0 ? row.models.join(', ') : '-') + '</td>';
						html += '<td>' + (row.inputTokens || 0).toLocaleString() + '</td>';
						html += '<td>' + (row.outputTokens || 0).toLocaleString() + '</td>';
						html += '<td>' + (row.cacheCreationTokens || 0).toLocaleString() + '</td>';
						html += '<td>' + (row.cacheReadTokens || 0).toLocaleString() + '</td>';
						...
						const projectName = row.session ? row.session.split('/').pop() : '-';
		...
		function addMcpFromTemplate() {
			...
			const templates = {
				'sqlite': {
					name: 'sqlite-db',
					command: 'npx',
					args: ['-y', '@modelcontextprotocol/server-sqlite', '--db', 'path/to/your/database.db'],
					env: {}
				},
				'filesystem': {
					name: 'filesystem',
					command: 'npx',
					args: ['-y', '@modelcontextprotocol/server-filesystem', '--read-only', 'c:/Users/LiuKe/Desktop'],
					env: {}
				},
				'github': {
					name: 'github',
				...
				'postgresql': {
					name: 'postgres-db',
					command: 'npx',
					args: ['-y', '@modelcontextprotocol/server-postgres', 'postgres://user:password@localhost/dbname'],
					env: {}
				},
				'fetch': {
					name: 'fetch',
		...
		function updateSettings() {
			...
			document.getElementById('mcpOptions').style.display = mcpEnabled ? 'block' : 'none';
